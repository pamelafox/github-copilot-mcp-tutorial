...
async def buy_product(
    ...
    if product["quantity"] < quantity:
        raise ToolError(f"Only {product['quantity']} units of '{product_name}' are in stock.")
    product["quantity"] -= quantity
    return f"Purchased {quantity}x {product_name} for ${total:.2f}. Remaining stock: {product['quantity']}."
```
